CREATE  PROCEDURE [dbo].[sp_OnboardingTimePerShippingChannel]
(
@fromdate DateTime,
@todate DateTime,
@idITBA int,
...
select 
dr.idITBA,b.ITBA,dr.CustomerName,s.shippingChannel, sum(n.timeSpent) 'MinutesSpent',CONVERT(VARCHAR(10),ROUND(sum(n.timeSpent)/60,0)) + 'Hrs ' + CONVERT(VARCHAR(10),CEILING(sum(n.timeSpent)%60)) + 'mins' TimeSpent
into #temp
...
join tblShippingChannel s on s.idShippingCHannel=dr.idShippingChannel
where n.noteDate >= @fromdate and n.noteDate <= @todate and n.activeFlag=1
and dr.activeFlag=1 and dr.idITBA is not null
group by dr.idITBA,b.ITBA,dr.CustomerName,s.shippingChannel
